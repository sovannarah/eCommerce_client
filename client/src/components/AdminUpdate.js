...
class Admin extends React.Component {
  ...
  async componentDidMount() {
    axios.get('http://10.34.6.23:8000/article')
      .then(res => {
  ...
  Getcategory() {
    return axios.get('http://10.34.6.23:8000/category')
      .then(res => {
        return (res.data)
      })
      .catch(err => {
  ...
  newCat() {
    ...
    axios.post('http://10.34.6.23:8000/category', data, { headers: this.state.headers })
      .then(res => {
        console.log(res.data)
      })
      .catch(err => {
            ...
            onRowUpdate: (newData, oldData) =>
              new Promise(resolve => {
                setTimeout(() => {
                  ...
                  axios.post(`http://10.34.6.23:8000/article/${data[data.indexOf(newData)].id}`, formData, { headers: this.state.headers })
                    .then(res => {
                      console.log(res.data)
                    })
                    .catch(console.log)
            ...
            onRowDelete: oldData =>
              new Promise(resolve => {
                setTimeout(() => {
                  ...
                  axios.delete(`http://10.34.6.23:8000/article/${remain[0].id}`,{headers:this.state.headers})
                  .then(res => {
                  console.log(res)
                  })
                  .catch(console.log)
